...
function ContextMenu({ x, y, onClose, ep, nav, speed, onSpeed }: CtxProps) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full px-4 py-3 text-sm flex items-center justify-center bg-white/10 text-white font-medium">
        Mp4Upload
      </div>
      {ep && (ep.previousEp || ep.nextEp || ep.backToDetails) && (
        <div className="border-t border-white/20 mt-2 pt-2">
          ...
          {ep.backToDetails && (
            <button
              onClick={() => nav(`/animerco/animes/${ep.backToDetails}`)}
              className="w-full px-4 py-3 text-sm text-gray-300 hover:bg-white/10 hover:text-white flex justify-between items-center"
              dir="rtl"
            >
...
export default function Mp4(): JSX.Element {
  ...
  const load = async () => {
    ...
    try {
      ...
      const videoUrl = html.slice(pos, html.indexOf('"', pos));
      setUrl(videoUrl);
    } catch {
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if (e.code === 'KeyN' && ep?.nextEp) {
        navigate(`/es/${ep.nextEp}`);
      }
      if (e.code === 'KeyV' && ep?.previousEp) {
        navigate(`/es/${ep.previousEp}`);
      }
      if (e.code === 'KeyB' && ep?.backToDetails) {
        navigate(`/animerco/animes/${ep.backToDetails}`);
      }
    };
      ...
      >
        ...
        {err ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-red-400">
            <p>{err}</p>
            <p className="text-sm opacity-70 mt-2">Right-click to open menu</p>
          </div>
        ) : (
          <video
            ref={vid}
