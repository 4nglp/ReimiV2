...
function Reader(): React.JSX.Element {
  const { mangaTitle, source, chapterPath } = useParams();
  ...
  useEffect(() => {
    const fetchChapter = async () => {
      if (!decodedChapterPath || !mangaTitle || !source) {
        setError('Chapter path or manga title/source not found');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (source === '3asq') {
          chapterContent = await getChapter3asq(decodedChapterPath, mangaTitle);
        } else if (source === 'lekmanga') {
          chapterContent = await getChapterLek(decodedChapterPath);
        }
    ...
    fetchChapter();
  }, [decodedChapterPath, mangaTitle, source]);
